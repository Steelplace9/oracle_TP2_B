...
BEGIN
  ...
  pkg_gestion_securisee_des_membres.modifier_coordonnees_membre(v_id_membre, 'COURRIEL', v_nouv_courriel);

  pkg_gestion_securisee_des_membres.modifier_coordonnees_membre(v_id_membre, 'TELEPHONE', v_nouv_tel);

  v_courriel_chiffre := pkg_gestion_securisee_des_membres.get_coordonnees_dechiffrees(v_id_membre, 'COURRIEL');

  DBMS_OUTPUT.PUT_LINE('Courriel déchiffré : ' || v_courriel_chiffre);

  v_tel_chiffre := pkg_gestion_securisee_des_membres.get_coordonnees_dechiffrees(v_id_membre, 'TELEPHONE');
