CREATE OR REPLACE PACKAGE pkg_gestion_securisee_des_membres AS
    ...
    TYPE r_membre_chiffre_type IS RECORD (
        couriel MEMBRES_SECURISES.COURRIEL_CHIFFRE%TYPE,
        telephone MEMBRES_SECURISES.TELEPHONE_CHIFFRE%TYPE
    );

    TYPE ref_membre IS REF CURSOR;

    TYPE t_resevations_type IS TABLE OF RESERVATIONS_ARCHIVEES%ROWTYPE;

    TYPE t_paiements_type IS TABLE OF PAIEMENTS_ARCHIVES%ROWTYPE;
...
CREATE OR REPLACE PACKAGE BODY pkg_gestion_securisee_des_membres AS
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ROLLBACK;
            DBMS_OUTPUT.PUT_LINE('une erreur est survenue l"aure de l"execution de la procedure copier_infos_chiffrees' || SQLERRM);
    ...
    EXCEPTION
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('une erreur est survenue l"aure de l"execution de la procedure ajouter_membre' || SQLERRM);
    END ajouter_membre;
    ...
    BEGIN
        SAVEPOINT avant_modification_membre;
        ...
        ELSE
            ROLLBACK TO SAVEPOINT avant_modification_membre;
    ...
    EXCEPTION
        ...
        WHEN OTHERS THEN
            ROLLBACK;
            DBMS_OUTPUT.PUT_LINE('une erreur est survenue l"aure de l"execution de la procedure modifier_coordonnees_membre' || SQLERRM);
    ...
    BEGIN
        IF p_champ_a_modifier = 'COURRIEL' THEN
            OPEN c_info FOR
                SELECT COURRIEL_CHIFFRE
                FROM MEMBRES_SECURISES
                WHERE MEMBRES_SECURISES.ID_MEMBRE = p_id_membre;
        ELSIF p_champ_a_modifier = 'TELEPHONE' THEN
            OPEN c_info FOR
    ...
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('aucune information n''a pas été trouver avec l''id donner');
        WHEN OTHERS THEN
            DBMS_OUTPUT.PUT_LINE('une erreur est survenue l"aure de l"execution de la procedure get_coordonnees_dechiffrees' || SQLERRM);
    ...
    BEGIN
        SAVEPOINT avant_modification_archive;

        SELECT ID_RESERVATION, DATE_RESERVATION, ID_MEMBRE, ID_ACTIVITE, SYSDATE BULK COLLECT INTO t_resvations FROM RESERVATIONS WHERE RESERVATIONS.ID_MEMBRE = p_id_membre;

        SELECT ID_PAIMENT, ID_MEMBRE, MONTANT, DATE_PAIMENT, TYPE_PAIMENT, SYSDATE BULK COLLECT INTO t_paiements FROM PAIMENTS WHERE PAIMENTS.ID_MEMBRE = p_id_membre;
        ...
        FORALL i IN t_resvations.FIRST .. t_resvations.LAST
            ...
            VALUES(t_resvations(i).ID_RESERVATION, t_resvations(i).DATE_RESERVATION, t_resvations(i).ID_MEMBRE, t_resvations(i).ID_ACTIVITE, SYSDATE);
        ...
        FORALL i IN t_paiements.FIRST .. t_paiements.LAST
            ...
            VALUES(t_paiements(i).ID_PAIEMENT, t_paiements(i).ID_MEMBRE, t_paiements(i).MONTANT, t_paiements(i).DATE_PAIEMENT, t_paiements(i).TYPE_PAIEMENT, SYSDATE);

        INSERT INTO MEMBRES_ARCHIVES VALUES(v_membre.ID_MEMBRE, v_membre.NOM_MEMBRE, v_membre.PRENOM_MEMBRE, v_membre.EMAIL_MEMBRE, v_membre.TELEPHONE_MEMBRE, v_membre.DATE_INSCRIPTION, v_membre.ID_ABONNEMENT, SYSDATE);

        FORALL i IN t_resvations.FIRST .. t_resvations.LAST
            DELETE RESERVATIONS WHERE ID_RESERVATION = t_resvations(i).ID_RESERVATION;

        FORALL i IN t_paiements.FIRST .. t_paiements.LAST
            DELETE PAIMENTS WHERE ID_PAIMENT = t_paiements(i).ID_PAIEMENT;

        COMMIT;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            ROLLBACK TO SAVEPOINT avant_modification_archive;
            DBMS_OUTPUT.PUT_LINE('aucune information n''a pas été trouver avec l''id donner');
        WHEN OTHERS THEN
            ROLLBACK;
            DBMS_OUTPUT.PUT_LINE('une erreur est survenue l"aure de l"execution de la procedure archiver_client_et_dependances' || SQLERRM);
